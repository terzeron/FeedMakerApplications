...
# 정규식을 적용한다. ep-row-v2-link anchor(href) 다음의 ep-row-v2-title >
# strong 텍스트를 (link, title)로 짝짓는다.
_ITEM_PATTERN = re.compile(
    r'<a class="ep-row-v2-link" href="(?P<link>/webtoon/\d+/[^"]+)">'
    ...
    if os.environ.get("TEST", ""):
        ...
        class TestCaptureItemSbxh(unittest.TestCase):
            ...
            def test_parse_args_f_ignored(self):
                self.assertEqual(
                    parse_args(["-f", "feed.xml"]), (1000, DEFAULT_URL_PREFIX)
                )

            def test_parse_args_url_prefix_override(self):
                self.assertEqual(
                    parse_args(["-u", "https://sbxh10.com/"]),
                    (1000, "https://sbxh10.com"),
                )

            def test_parse_args_combined(self):
                self.assertEqual(
                    parse_args(["-f", ".", "-n", "3"]), (3, DEFAULT_URL_PREFIX)
                )
            ...
            def _item(self, link, title):
                return (
                    f'<a class="ep-row-v2-link" href="{link}">'
                    f'<div class="ep-row-v2-thumb"><img src="x"/>'
                    f'<span class="ep-row-v2-no">1</span></div>'
                    f'<div class="ep-row-v2-body"><div class="ep-row-v2-title">'
                    f"<strong>{title}</strong></div></div></a>"
                )

            def test_parse_feed_list_basic(self):
                content = self._item(
                    "/webtoon/57369982/1237502", "1년차 만렙 매니저 97화"
                )
                self.assertEqual(
                    parse_feed_list(content, DEFAULT_URL_PREFIX),
                    [
                        (
                            "https://sbxh9.com/webtoon/57369982/1237502",
                            "1년차 만렙 매니저 97화",
                        )
                    ],
                )
            ...
            def test_parse_feed_list_nonnumeric_epid(self):
                content = self._item(
                    "/webtoon/57369982/kp-57369982-69705723", "1년차 만렙 매니저 196화"
                )
                self.assertEqual(
                    parse_feed_list(content, DEFAULT_URL_PREFIX),
                    [
                        (
                            "https://sbxh9.com/webtoon/57369982/kp-57369982-69705723",
                            "1년차 만렙 매니저 196화",
                        )
                    ],
                )
            ...
            def test_parse_feed_list_multiple_preserves_order(self):
                content = self._item("/webtoon/1/196", "196화") + self._item(
                    "/webtoon/1/195", "195화"
                )
                self.assertEqual(
                    parse_feed_list(content, "https://sbxh9.com"),
                    [
                        ("https://sbxh9.com/webtoon/1/196", "196화"),
                        ("https://sbxh9.com/webtoon/1/195", "195화"),
                    ],
                )

            def test_parse_feed_list_collapses_title_whitespace(self):
                content = self._item(
                    "/webtoon/1/1", "  0097 -  1년차   만렙&nbsp;매니저 97화 "
                )
                self.assertEqual(
                    parse_feed_list(content, "https://sbxh9.com"),
                    [
                        (
                            "https://sbxh9.com/webtoon/1/1",
                            "0097 - 1년차 만렙 매니저 97화",
                        )
                    ],
                )
